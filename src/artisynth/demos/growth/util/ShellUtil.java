...
public class ShellUtil {
   ...
   public static double area (FemNode3d[] nodes, boolean isRest) {
      if (isRest) {
         return MathUtil
            .area (
               nodes[0].getRestPosition (), nodes[1].getRestPosition (),
   ...
   /**
    * Get the average plastic deformation gradient across the integration points
    * of an element.
    */
   ...
   /**
    * Create an element's jacobian matrix that describes the change of some node
    * quantity (e.g. world-space velocity) with respect to the change in local
    * coordinates of the shell element.
    * 
    * Formulation is shown in equation A.2 of the thesis paper "Large Growth
    * Deformations of Thin Tissue using Solid-Shells". Replace x_a and y_a with
    * the front and back node quantities.
    * 
    * @param iPt
    ...
    */
   public static Matrix3d createCustomJacobian (
      IntegrationPoint3d iPt, FemNode3d[] nodes, Vector3d[] frontVals,
      Vector3d[] backVals) {
      // POST_CORRECT
    ...
    */
   public static Point3d computePositionAtIntegrationPoint (
      int k, ShellElement3d ele) {
   ...
   public static boolean detectOverlappingNode (FemModel3d femModel) {
      for (FemNode3d n0 : femModel.getNodes ()) {
         for (FemNode3d n1 : femModel.getNodes ()) {
            if (n0 != n1 && n0
               .getRestPosition ()
               .distance (n1.getRestPosition ()) < MathUtil.ELIPSON) {
               System.out
                  .println (
                     "Overlapping Node: " + n0.getName () + " "
                     + n1.getName ());
               return true;
            }
         }
   ...
   public static boolean detectNegativeInverseRestJacobian (
      ...
      for (ShellElement3d ele : femModel.getShellElements ()) {
         ...
         for (int k = 0; k < ipts.length; k++) {
            double invDetJ =
               idts[k].computeInverseRestJacobian (ipts[k], ele.getNodes ());
   ...
   public static Point3d getPosition (
      FemNode3d node, boolean isFront, boolean isWorld) {
      return (isFront && isWorld) ? node.getPosition ()
         : (isFront && !isWorld) ? node.getRestPosition ()
            : (!isFront && isWorld) ? node.getBackPosition ()
               : node.getBackRestPosition ();
   }

   public static Point3d getAvgPosition (
      LinkedList<FemNode3d> nodes, boolean isFront, boolean isWorld) {
   ...
   public static void setPosition (
      FemNode3d node, boolean isFront, boolean isWorld, Point3d newPos) {
      if (isFront && isWorld)
   ...
   public static double getDihedralAngle (
      FemModel3d model, HalfEdge edge, boolean isRest) {
   ...
   public static LinkedList<FemElement3dBase> getAllElements (
      FemModel3d model) {
