...
public class MeshUtil {
   ...
   /**
    * Given a half edge, return either the same half edge or its sibling half
    * edge. The half edge that is returned is the one with the smallest head
    * vertex index.
    * 
   ...
   public static boolean isShareVertex (HalfEdge he0, HalfEdge he1) {
      return (he0.head == he1.head || he0.head == he1.tail
      || he0.tail == he1.head || he0.tail == he1.tail);
   }
   ...
   public static Face createFace (
      Vertex3d vtx0, Vertex3d vtx1, Vertex3d vtx2, Face refFace) {
    ...
    */
   public static Face createFace (
      Vertex3d vtx0, Vertex3d vtx1, Vertex3d vtx2, Vector3d refNrm) {
   ...
   /**
    * A set of ordered vertices is considered counter clockwise if they form a
    * face that has a normal that faces upwards (+z).
    */
   ...
   public static Vertex3d[] getOppositeVtxs (HalfEdge he) {
      if (he.opposite != null) {
         return new Vertex3d[] { he.getNext ().head,
   ...
   public static Vertex3d getCommonVertex (HalfEdge he0, HalfEdge he1) {
      if (he0.head == he1.head || he0.head == he1.tail) {
         return he0.head;
      }
      else if (he0.tail == he1.head || he0.tail == he1.tail) {
         return he0.tail;
      }
      else {
         return null;
    ...
    */
   public static Vertex3d getOtherVertex (
      HalfEdge knownEdge, HalfEdge connEdge) {
   ...
   public static Face getAdjacentFace (
      ...
      for (int e = 0; e < 3; e++) {
         ...
         if (oppFace != null && MeshUtil.isHasVertex (reqVtx, oppFace)
         && oppFace != excludeFace) {
            return oppFace;
         }
      }
   ...
   /**
    * Given two faces (specified by their common edge), return the four vertices
    * of the two faces.
    */
    ...
    */
   public static LinkedList<HalfEdge> getAllEdgesViaCopy (
      ...
      for (Face face : activeFaces) {
         for (int e = 0; e < 3; e++) {
            ...
            if (!edge.checkFlag (visitedFlag) && (edge.opposite == null
            || !edge.opposite.checkFlag (visitedFlag))) {
    ...
    * 
    * @return null if faces are not adjacent.
    */
   ...
   public static int numEdges (PolygonalMesh mesh) {
      ...
      for (Face evenFace : mesh.getFaces ()) {
         for (int h = 0; h < 3; h++) {
            HalfEdge he =
               MeshUtil.getHalfEdgeWithMinHeadIdx (evenFace.getEdge (h));
   ...
   public static PolygonalMesh createHexagonalTriRectangle (
      double xLen, double yLen, int xDiv, int yDiv,
      boolean isShaveHalfTriangles) {
      ...
      int numVtxs =
         ((yDiv + 1) / 2 + 1) * numVtxsPerNonDiagVtxRow
         + (yDiv / 2) * numVtxsPerDiagVtxRow;
      ...
      for (int vr = 0; vr < yDiv + 1; vr++) {
         ...
         int numVtxCols =
            (isDiagVtxRow) ? numVtxsPerDiagVtxRow : numVtxsPerNonDiagVtxRow;

         double y = vr * yDivLen;

         for (int vc = 0; vc < numVtxCols; vc++) {
            double x =
               (vc == 0) ? 0 : (vc == numVtxCols - 1) ? xLen : (isDiagVtxRow)
                  ? vc * xDivLen : xDivLen / 2 + (vc - 1) * xDivLen;
      ...
      for (int fr = 0; fr < yDiv; fr++) {
         ...
         int bv =
            ((fr + 1) / 2) * numVtxsPerNonDiagVtxRow
            + ((fr) / 2) * numVtxsPerDiagVtxRow;
         int tv =
            ((fr + 2) / 2) * numVtxsPerNonDiagVtxRow
            + ((fr + 1) / 2) * numVtxsPerDiagVtxRow;
   ...
   public static PolygonalMesh createUniformDisc (int res, int circRes) {
      ...
      for (int circ = 0; circ < res; circ++) {
         ...
         for (int point = 0; point < numTri; point++) {
            Vertex3d vtx =
               new Vertex3d (
                  Math.cos (angleStep * point), Math.sin (angleStep * point),
                  0);
      ...
      for (int i = 0; i < faceIdxs.size (); i += 3)
         mesh
            .addFace (
               new int[] { faceIdxs.get (i), faceIdxs.get (i + 1),
                           faceIdxs.get (i + 2) });
