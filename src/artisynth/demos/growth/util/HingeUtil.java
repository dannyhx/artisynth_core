...
public class HingeUtil {
   ...
   public static Matrix3d computeMembraneStrain (ShellTriElement ele) {
      ...
      for (NeighborElement neigh : adjFaces) {
         ...
         Vector3d edgeVecRest =
            new Vector3d (nodeH.getRestPosition ())
               .sub (nodeT.getRestPosition ());
         double l = edgeVec.norm ();
         double lr = edgeVecRest.norm ();

         Vector3d t =
            HingeUtil.computeVecT (ele, eleNOR, edgeVec, nodeTPos, false);
   ...
   protected static Vector3d computeVecT (
      ...
      Point3d edgeCenter =
         (Point3d)new Point3d (edgeNodeTPos).scaledAdd (0.5, edgeVec);
