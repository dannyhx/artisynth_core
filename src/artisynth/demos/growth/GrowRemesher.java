...
public class GrowRemesher extends ShellRemesher {
    ...
    * 
    * mFps_prev[f*numEles+k] = plastic deformation gradient stored in element e
    * at integration data k. Indexed by face index `f`.
    */
   ...
   /**
    * Distance offset when searching for nearby integration points of the old
    * mesh.
    */
   ...
   double refineVelocity, double refineMorphogen, MeshChemicals meshChems) {
      super (
         mesh, femModel, sizeMin, sizeMax, aspectMin, refineAngle, refineComp,
         refineVelocity);
   ...
   public void setTarget (
      PolygonalMesh mesh, FemModel3d femModel, MeshChemicals meshChems) {
   ...
   public void preRemesh () {
      ...
      for (int f = 0; f < mMesh.numFaces (); f++) {
         ...
         for (int k = 0; k < numIpnts; k++) {
            ...
            Point3d ipnt_pos =
               ShellUtil.computePositionAtIntegrationPoint (k, gEle);
            mIntegPos_prev[f * numIpnts + k] =
               new Index3d (ipnt_pos, f * numIpnts + k);
         }
      }
   }
      ...
      for (int e = 0; e < newEles.size (); e++) {
         ...
         for (int k = 0; k < numK; k++) {
            ...
            if (mInterpolationMode.equals ("nearest")) {
               new_k_Fp =
                  interpolatePlasticDeformationGradient_nearestNeigh (
   ...
   protected Matrix3d interpolatePlasticDeformationGradient_nearestNeigh (
      ...
      ArrayList<Point3d> neighPnts =
         nearestNeighInterpolator
            .nearestNeighbourSearch (samplePnt, mNumNeighs, mNeighTol);
      ...
      for (int p = 0; p < mNumNeighs; p++) {
         ...
         if (Double.isInfinite (wts[p])) {
            ...
            itpPrestrain
               .scaledAdd (wts[p], this.mFps_prev[f * numIntegPts + k]);
         }
      }
    ...
    * 
    * @param v0
    * v1 Nodes whose attributes are to be averaged together.
    * 
    * @param vTgt
   ...
   protected void setVertexViaLinearInterpolation (
      ...
      if (n0.mIsMorphogenSrc && n1.mIsMorphogenSrc) {
         ((GrowNode3d)mFemModel.getNode (vTgt.getIndex ())).mIsMorphogenSrc =
            true;
      }

      if (n0.mIsNoMorphogenZone && n1.mIsNoMorphogenZone) {
         ((GrowNode3d)mFemModel.getNode (vTgt.getIndex ())).mIsNoMorphogenZone =
            true;
      }

      if (!n0.isDynamic () && !n1.isDynamic ()) {
         ((GrowNode3d)mFemModel.getNode (vTgt.getIndex ())).setDynamic (false);
      }
   }
   ...
   public void cutNodes (LinkedList<FemNode3d> nodeSeq) {
      ...
      for (int n = 0; n < seqLen - 1;) {
         Vertex3d vtx0 =
            mMesh
               .getVertex (
                  Integer.parseInt (nodeSeq.get (n).getName ().substring (8)));
         Vertex3d vtx1 =
            mMesh
               .getVertex (
                  Integer
                     .parseInt (nodeSeq.get (n + 1).getName ().substring (8)));
         ...
         if (isBnd0 && isBnd1) {
            ...
            throw new RuntimeException (
               "Unnecessary incision case. Use 3 "
      ...
      // Are first and last nodes in sequence connected?
      {
         Vertex3d firstVtx =
            mMesh
               .getVertex (
                  Integer.parseInt (nodeSeq.get (0).getName ().substring (8)));
         Vertex3d lastVtx =
            mMesh
               .getVertex (
                  Integer
         ...
         if (MeshUtil.isConnected (firstVtx, lastVtx)) {
            cutNodes (
               new Vertex3d[] { firstVtx, lastVtx, null }, "b-b", twinVtxs);
         }
      }
    ...
    * 
    * @param cutCase
    * "b-b" = seqVtxs specifies 2 boundary vertices. "b-n" = seqVtxs specifies 1
    * boundary vertex and 1 non-boundary vertex. "n-n-n" = seqVtxs specifies 3
    * non-boundary vertices.
    * 
    * @param twinVtxs
    * Keeps track of seqVtxs and their duplicates. Used for separating these
    * vertices/nodes apart later.
    */
   public void cutNodes (
      Vertex3d[] seqVtxs, String cutCase, TwinVertexCollection twinVtxs) {
         ...
         for (int d = 0; d < numDir; d++) {
            ...
            Vertex3d revolVtx =
               (cutCase.equals ("b-n")) ? seqVtxs[0]
                  : (cutCase.equals ("b-b")) ? seqVtxs[d] : seqVtxs[1]; // n-n-n
                                                                        // case
            ...
            for (int i = 0; true; i++) {
               if (curFace == null) {
                  ...
                  curFace =
                     (s == 0) ? edge0.getFace () : edge0.getOppositeFace ();
                  prvFace =
                     (s == 0) ? edge0.getOppositeFace () : edge0.getFace ();
               }
               else {
                  Face nxtFace =
                     MeshUtil
                        .getAdjacentFace (
                           curFace, prvFace/* excludeFace */,
                           revolVtx/* reqVtx */);
                  prvFace = curFace;
                  curFace = nxtFace;
               }
      ...
      if (cutCase.equals ("b-n")) {
         dupSeqVtxs[0] =
            addVertexViaLinearInterpolation (seqVtxs[0], seqVtxs[0]);
         ...
         dupSeqVtxs[0] =
            addVertexViaLinearInterpolation (seqVtxs[0], seqVtxs[0]);
         dupSeqVtxs[1] =
            addVertexViaLinearInterpolation (seqVtxs[1], seqVtxs[1]);
      }
      else {
         dupSeqVtxs[1] =
            addVertexViaLinearInterpolation (seqVtxs[1], seqVtxs[1]);
      ...
      for (Face refFace : travFaces_s1) {
         ...
         Face newFace =
            MeshUtil
               .createFace (
                  newFaceVtxs[0], newFaceVtxs[1], newFaceVtxs[2], refFace);
   ...
   public class TwinVertex {
      ...
      protected void calculateSeparation (
         String whichVtx, LinkedList<Vertex3d> adjVtxsToIgnore) {
         ...
         for (boolean isFront : new boolean[] { true, false }) {
            for (boolean isWorld : new boolean[] { true, false }) {
               ...
               Point3d posB =
                  ShellUtil.getAvgPosition (validAdjNodes, isFront, isWorld);
