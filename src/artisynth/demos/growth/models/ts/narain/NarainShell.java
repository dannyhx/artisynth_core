...
public class NarainShell extends ThinShellBase {
   ...
   protected void setDDEMaterial (String matName) {
      ...
      if (matName == "paper") {
         ...
         b
            .setRow (
               0, new double[] { 117.070122e-6, 202.682922e-6, 298.615936e-6,
                                 244.588867e-6, 238.437820e-6 });
         b
            .setRow (
               1, new double[] { 87.509773e-6, 36.396919e-6, 103.628098e-6,
                                 91.630119e-6, 97.175949e-6 });
         b
            .setRow (
               2, new double[] { 59.774277e-6, 59.260632e-6, 64.199661e-6,
                                 55.017326e-6, 59.221714e-6 });
      }
      else {
   ...
   /**
    * Create the indirect node neighbors that's needed to associate the opposite
    * vertices of each triangle pair in terms of stiffness.
    * 
   ...
   public void addStretchingForceAndStiffness () {
      for (Face face : mMesh.getFaces ()) {
         Pair<MatrixNd,VectorNd> fs =
            computeStretchingForcesAndStiffness (face);
         MatrixNd stiffness = fs.first;
         VectorNd force = fs.second;

         FemNode3d[] nodes =
            mModel.getShellElement (face.getIndex ()).getNodes ();
   ...
   /**
    * Calculate the nodal bending forces for the given edge. The 4 nodes (2 from
    * edge and 2 that are opposite of the edge) will have their forces and
    * stiffness determined.
    * 
    * [0] -> Edge Head Node [1] -> Edge Tail Node [2] -> Opposite Node [3] ->
    * Opposite Node of Opposite Face.
    * 
   ...
   protected Pair<MatrixNd,VectorNd> computeBendingForceAndStiffness (
      ...
      ShellTriElement ele0 =
         (ShellTriElement)mModel.getShellElement (edge.getFace ().getIndex ());
      ShellTriElement ele1 =
         (ShellTriElement)mModel
            .getShellElement (edge.getOppositeFace ().getIndex ());
   ...
   protected Pair<MatrixNd,VectorNd> computeStretchingForcesAndStiffness (
      ...
      Point3d[] xs =
         new Point3d[] { nodes[0].getPosition (), nodes[1].getPosition (),
                         nodes[2].getPosition () };
      ...
      VectorNd[] fs =
         new VectorNd[] { new VectorNd (3), new VectorNd (3),
                          new VectorNd (3) };
         ...
         for (int i = 0; i < 3; i++) {
            for (int j = 0; j <= i; j++) {
               ...
               if (i == j) {
                  ...
                  grad_f.scaledAdd (gf * this.mAltPoisson * G.trace (), dG);
               }
               else {
                  grad_f
                     .scaledAdd (
               ...
               if (i == j) {
                  MatrixNd d_trace_c_x_dG =
                     MathUtil.outerProduct (d_trace_c, dG);
    ...
    * min() is called on both. Makes possible assumption adjacent elements do
    * not share nodes, and that adjacent elements have their own material model.
    * 
    * @param edge
    * @param theta
    * @return
    ...
    * @return
    */
   protected double bending_stiffness_dde (
      HalfEdge edge, MatrixNd data, double a, double l, double theta) {
      ...
      if (biasAngle < 0)
         biasAngle = -biasAngle;
      if (biasAngle > 4)
         biasAngle = 8 - biasAngle;
      if (biasAngle > 2)
         biasAngle = 4 - biasAngle;
               ...
               for (int l = 0; l < 4; l++) {
                  ...
                  stiffness
                     .set (
                        l,
                        lval
                        + mMatDDEStretchingSamples[ai + i][bi + j][ci + k][l]
                        * weight[i][j][k]);
               }
   ...
   protected Pair<Matrix3d,Matrix3d> deformation_gradient (Face face) {
      ...
      ShellTriElement ele =
         (ShellTriElement)mModel.getShellElement (face.getIndex ());
   ...
   /**
    * Convert a matrix-representation of the given face's edge strain into a
    * Vec3 where each element corresponds to an edge.
    * 
   ...
   public Vector3d bendStrain_faceToEdges (Face face, Matrix3d S) {
      ...
      for (int e = 0; e < 3; e++) {
         ...
         A
            .setColumn (
               e, new double[] { Se.get (0, 0), Se.get (1, 1), Se.get (2, 2),
      ...
      VectorNd y =
         new VectorNd (
            S.get (0, 0), S.get (1, 1), S.get (2, 2), S.get (0, 1),
            S.get (0, 2), S.get (1, 2));
      y.scale (areaRest);
   ...
   public void remeshRemoveFacePreOp (Face face, OpRv opRv) {
      ShellTriElement rmEle =
         (ShellTriElement)mModel.getShellElement (face.idx);
   ...
   public void applyGrowthTensorToEle (
      ...
      if (isBendingMorphogenHack) {
         ...
         Vector3d edgeStrains = bendStrain_faceToEdges (face, avgStrain);
