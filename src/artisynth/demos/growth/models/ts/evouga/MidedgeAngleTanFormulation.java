...
public class MidedgeAngleTanFormulation {
    ...
    * Verified.
    * 
    * Algorithm: For each edge e: 1. Calculate the triangle's altitude (i.e.
    * unnormalized normal vector length with respect to edge's length) 2.
    * Calculate bend angle at edge. 3. II[e] = 2 * altitude * tan(0.5 * theta)
    * 
    * @param f
    * @param derivative
    * 3x21
   ...
   public static Vector3d secondFundamentalFormEntries (
      FemModel3d model, VectorNd extraDOFs, MeshConnectivity MC, Face face,
      ...
      for (int i = 0; i < 3; i++) {
         ...
         double altitude =
            GeometryDerivative
               .triangleAltitude (
                  MC, model, face, i, (derivative != null) ? hderiv : null,
         ...
         double theta =
            edgeTheta (
               MC, model, gEdgeIdx, (derivative != null) ? thetaderiv : null,
         ...
         if (derivative != null) {
            ...
            derivative
               .addScaledSubMatrix (i, 3 * hv0, 2.0 * tan (alpha), block);

            hderiv.getSubMatrix (0, 3, block);
            derivative
               .addScaledSubMatrix (i, 3 * hv1, 2.0 * tan (alpha), block);

            hderiv.getSubMatrix (0, 6, block);
            derivative
               .addScaledSubMatrix (i, 3 * hv2, 2.0 * tan (alpha), block);
            ...
            derivative
               .addScaledSubMatrix (
                  i, 3 * av0, altitude / cos (alpha) / cos (alpha), block);

            thetaderiv.getSubMatrix (0, 3, block);
            derivative
               .addScaledSubMatrix (
                  i, 3 * av1, altitude / cos (alpha) / cos (alpha), block);

            thetaderiv.getSubMatrix (0, 6, block);
            derivative
               .addScaledSubMatrix (
                  i, 3 * av2, altitude / cos (alpha) / cos (alpha), block);

            thetaderiv.getSubMatrix (0, 9, block);
            derivative
               .addScaledSubMatrix (
                  i, 3 * av3, altitude / cos (alpha) / cos (alpha), block);
         ...
         if (hessian != null) {
            ...
            for (int j = 0; j < 3; j++) {
               for (int k = 0; k < 3; k++) {
                  ...
                  hessian[i]
                     .addScaledSubMatrix (
                        3 * hv[j], 3 * hv[k], 2.0 * tan (alpha), block);
               }
            }
            ...
            for (int k = 0; k < 3; k++) {
               ...
               for (int j = 0; j < 4; j++) {
                  ...
                  hessian[i]
                     .addScaledSubMatrix (
                        3 * av[j], 3 * hv[k], 1.0 / cos (alpha) / cos (alpha),
                        block);

                  block.mulTransposeLeft (hderiv_block, thetaderiv_block);
                  hessian[i]
                     .addScaledSubMatrix (
                        3 * hv[k], 3 * av[j], 1.0 / cos (alpha) / cos (alpha),
                        block);
               }
            ...
            for (int k = 0; k < 4; k++) {
               ...
               hessian[i]
                  .addScaledSubMatrix (
                     18 + i, 3 * av[k], 2.0 * altitude * tan (alpha)
                     / cos (alpha) / cos (alpha) * orient, thetaderiv_block);

               block.set (thetaderiv_block);
               block.transpose ();
               hessian[i]
                  .addScaledSubMatrix (
                     3 * av[k], 18 + i, 2.0 * altitude * tan (alpha)
                     / cos (alpha) / cos (alpha) * orient, block);
            }
            ...
            hessian[i]
               .add (
                  18 + i, 18 + i,
                  4.0 * altitude * tan (alpha) / cos (alpha) / cos (alpha));
         }
      }
   ...
   public static Matrix2d secondFundamentalForm (
      ...
      Vector3d II =
         secondFundamentalFormEntries (
            model, extraDOFs, MC, face, derivative != null ? IIderiv : null,
            hessian != null ? IIhess : null);

      Matrix2d result = new Matrix2d (II.x + II.y, II.x, II.x, II.x + II.z);
   ...
   protected static double edgeTheta (
      MeshConnectivity MC, FemModel3d model, int edge, MatrixNd derivative,
      MatrixNd hessian) {
      if (derivative != null) {
         derivative.setZero ();
      }
