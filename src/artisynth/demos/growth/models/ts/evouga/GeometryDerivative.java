...
public class GeometryDerivative {
   ...
   public static Matrix2d firstFundamentalForm (
      MeshConnectivity MC, FemModel3d model, int f, MatrixNd derivative,
      MatrixNd[] hessian) {
      mTimerI.restart ();
      ...
      Matrix2d result =
         new Matrix2d (
            q1_q0.dot (q1_q0), q1_q0.dot (q2_q0), q2_q0.dot (q1_q0),
            q2_q0.dot (q2_q0));

      if (derivative != null) {
         derivative.setZero ();
         MatrixUtil
            .add1x3Block (derivative, 0, 3, new Vector3d (q1_q0).scale (2));
         MatrixUtil
            .add1x3Block (derivative, 0, 0, new Vector3d (q1_q0).scale (-2));
         MatrixUtil.add1x3Block (derivative, 1, 6, new Vector3d (q1_q0));
         MatrixUtil.add1x3Block (derivative, 1, 3, new Vector3d (q2_q0));
         MatrixUtil
            .add1x3Block (
               derivative, 1, 0, new Vector3d (q1_q0).add (q2_q0).scale (-1));
         MatrixUtil.add1x3Block (derivative, 2, 6, new Vector3d (q1_q0));
         MatrixUtil.add1x3Block (derivative, 2, 3, new Vector3d (q2_q0));
         MatrixUtil
            .add1x3Block (
               derivative, 2, 0, new Vector3d (q1_q0).add (q2_q0).scale (-1));
         MatrixUtil
            .add1x3Block (derivative, 3, 6, new Vector3d (q2_q0).scale (2));
         MatrixUtil
            .add1x3Block (derivative, 3, 0, new Vector3d (q2_q0).scale (-2));
      }
    ...
    * @param ele
    * @param edgeIdx
    * One of the three edges of the given element. 0,1,2. i.e. local edgeIdx
    * relative to element.
    * 
    * @param derivative
    * 1x9
   ...
   public static double triangleAltitude (
      ...
      Vector3d n =
         faceNormal (
            MC, model, face, edgeIdx, (derivative != null) ? nderiv : null,
      ...
      if (derivative != null) {
         ...
         eT.setRow (0, e);
         derivative
            .addScaledSubMatrix (0, 6, -nnorm / enorm / enorm / enorm, eT);
         derivative
            .addScaledSubMatrix (0, 3, nnorm / enorm / enorm / enorm, eT);
      }
      ...
      if (hessian != null) {
         ...
         hessian
            .addScaledSubMatrix (
               6, 0, -1 * nnorm / enorm / enorm / enorm, block);
         hessian
            .addScaledSubMatrix (
               3, 0, +1 / nnorm / enorm / enorm / enorm, block);
         ...
         hessian
            .addScaledSubMatrix (
               0, 6, -1.0 / nnorm / enorm / enorm / enorm, block);
         hessian
            .addScaledSubMatrix (
               0, 3, +1.0 / nnorm / enorm / enorm / enorm, block);

         block.set (Matrix3d.IDENTITY);
         hessian
            .addScaledSubMatrix (
               6, 6, -1.0 * nnorm / enorm / enorm / enorm, block);
         hessian
            .addScaledSubMatrix (
               6, 3, +1.0 * nnorm / enorm / enorm / enorm, block);
         hessian
            .addScaledSubMatrix (
               3, 6, +1.0 * nnorm / enorm / enorm / enorm, block);
         hessian
            .addScaledSubMatrix (
               3, 3, -1.0 * nnorm / enorm / enorm / enorm, block);
   ...
   public static double angle (
      Vector3d v, Vector3d w, Vector3d axis, MatrixNd derivative,
      MatrixNd hessian) {
      ...
      if (derivative != null) {
         ...
         derivative
            .addScaledSubMatrix (
               0, 0, -1.0 / v.normSquared () / axis.norm (), axisV_1x3);
         derivative
            .addScaledSubMatrix (
               0, 3, +1.0 / w.normSquared () / axis.norm (), axisW_1x3);
      ...
      if (hessian != null) {
         ...
         hessian
            .addScaledSubMatrix (
               0, 0, 2.0 / v.normSquared () / v.normSquared () / axis.norm (),
               block);
         ...
         hessian
            .addScaledSubMatrix (
               3, 3, -2.0 / w.normSquared () / w.normSquared () / axis.norm (),
               block);

         block = MatrixUtil.crossMatrixNd (axis);
         hessian
            .addScaledSubMatrix (
               0, 0, -1.0 / v.normSquared () / axis.norm (), block);

         hessian
            .addScaledSubMatrix (
               3, 3, 1.0 / w.normSquared () / axis.norm (), block);
         ...
         dahat
            .addScaledOuterProduct (
               -1.0 / axis.norm () / axis.norm () / axis.norm (), axis, axis);

         // Last 2 blocks

         block.mul (MatrixUtil.crossMatrix (v), dahat);
         hessian.addScaledSubMatrix (0, 6, 1.0 / v.normSquared (), block);

         block.mul (MatrixUtil.crossMatrix (w), dahat);
         hessian.addScaledSubMatrix (3, 6, -1.0 / w.normSquared (), block);
      }
