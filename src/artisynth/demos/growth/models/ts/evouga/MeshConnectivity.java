...
public class MeshConnectivity {
   ...
   /**
    * FEorient[f][local_e] = 0 if local_e half-edge belongs to f. Otherwise, 1
    * if half-edge belongs to opposite f.
    */
   public int[][] FEorient;
   ...
   public void refresh () {
      ...
      LinkedHashMap<String,int[]> edgeFaces =
      ...
      for (int f = 0; f < nfaces; f++) {
         for (int j = 0; j < 3; j++) {
            ...
            if (edgeFaces.get (p) == null) {
               int[] hef = new int[2];
               hef[idx] = f;
               hef[1 - idx] = -1; // To be populated when iterate to other
                                  // half-edge, if exists.
               edgeFaces.put (p, hef);
            }
            else {
