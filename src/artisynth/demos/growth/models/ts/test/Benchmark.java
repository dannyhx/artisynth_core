 ...
 * 
 * Set Artisynth command-line flags to include:-play Note that GUI runs in
 * separate thread; doesn't impact solver performance.
 * 
 * As set FemModel3d.profileStressAndStiffness to true;
 * 
 * Benchmark will run for 1 seconds. See the "Time Elapsed" in the console when
 * run finishes.
 * 
 * 1012 elements (22x23): Solid-shell: Thin-shell:
 * 
 * 5000 elements (50x50): Solid-shell: Thin-shell:
 * 
   ...
   public void advanceCustom_applyMorphogen (double t0) {
      for (int v = 0; v < mMesh[0].numVertices (); v++) {
         if (isMorphogenSrcNode (v) && t0 < morphogenSrcDuration) {
            ...
            gNode.mChems
               .set (3, (this.mEleClass == ElementClass.MEMBRANE) ? 0.1 : 1);
         }
      }
   }
}
