...
/**
 * Square patch of triangular solid-shell elements, subjected to gravity. Some
 * nodes will be held in-place to demonstrate shell bending forces.
 * 
...
public class ShellPatch extends RootModel {
   ...
   /**
    * Create a mesh, which will be used as a template for building the FEM
    * model.
    */
   protected void build_modelSkeleton () {
      ...
      for (int m = 0; m < M; m++) {
         mMesh[m] =
            MeshFactory.createPlane (mMeshX, mMeshY, mMeshXDiv, mMeshYDiv);
      }
   }
      ...
      for (int m = 0; m < M; m++) {
         ...
         for (int f = 0; f < mMesh[m].numFaces (); f++) {
            ...
            if (mEleClass == ElementClass.VOLUMETRIC) {
               FemNode3d n3 =
                  mFemModel[m].getNode (vtxIdxs[0] + mMesh[m].numVertices ());
               FemNode3d n4 =
                  mFemModel[m].getNode (vtxIdxs[1] + mMesh[m].numVertices ());
               FemNode3d n5 =
                  mFemModel[m].getNode (vtxIdxs[2] + mMesh[m].numVertices ());
               ...
               ShellTriElement ele =
                  createShellElement (n0, n1, n2, m_shellThickness);
               ele.setName ("MyEle_#" + f);
         ...
         if (mEleClass == ElementClass.MEMBRANE) {
            mFemModel[m].myThinShellAux =
               new ThinShellAux (mTsType, mFemModel[m], mMesh[m]);
            mFemModel[m].myThinShellAux
   ...
   /**
    * Initialize objects, such as those that require the FEM model to be already
   ...
   protected void build_femRendering () {
      for (int m = 0; m < M; m++) {
         ...
         RenderProps
            .setShininess (
               mFemModel[m],
               mFemModel[m].getRenderProps ().getShininess () * 10);
         RenderProps.setVisible (mFemModel[m], true);
         RenderProps
            .setFaceStyle (mFemModel[m], Renderer.FaceStyle.FRONT_AND_BACK);
         RenderProps
            .setPointStyle (
               mFemModel[m].getNodes (), Renderer.PointStyle.SPHERE);
         RenderProps
            .setPointRadius (mFemModel[m].getNodes (), mRendCfg.mNodeRadius);
   ...
   protected void build_assembleUI () {
      ...
      LabeledComponentBase comp =
         mPanel.addWidget (mFemModel[0], "material.YoungsModulus");
   ...
   protected ShellTriElement createShellElement (
      FemNode3d n0, FemNode3d n1, FemNode3d n2, double thickness) {
      return new ShellTriElement (
         n0, n1, n2, thickness, mEleClass == ElementClass.MEMBRANE);
   }
   ...
   static {
      myProps
         .add (
            "shellThickness", "Thickness of each shell element",
            m_shellThickness);
   }
   ...
   public void setShellThickness (double newThickness) {
      if (mEleClass == ElementClass.VOLUMETRIC) {
         System.out
            .println ("Skipping thickness adjustment for volumetric elements.");
         return;
      }
