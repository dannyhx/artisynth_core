...
public class Intricate_Cylinder extends GrowDemo {
   ...
   protected enum Type {
      LOWRES_TOP_CYLINDER, HIGHRES_TOP_CYLINDER, PILLOW, BUD
   }
   ...
   protected void build_pre () {
      ...
      mSizeMin =
         (mType == Type.PILLOW) ? 0.1 : (mType == Type.HIGHRES_TOP_CYLINDER)
            ? 0.1 : (mType == Type.LOWRES_TOP_CYLINDER) ? 0.2 : // 0.2
               (mType == Type.BUD) ? 0.1 : 0.02;
         ...
         (mType == Type.PILLOW)
            ...
            : (mType == Type.HIGHRES_TOP_CYLINDER)
               ...
               : (mType == Type.LOWRES_TOP_CYLINDER)
                  ...
                  : (mType == Type.BUD)
                     ? MeshFactory.createIcosahedralSphere (0.5, 4) : null;
   }
   ...
   public boolean isMorphogenSrcNode (int v) {
      ...
      return (mType == Type.LOWRES_TOP_CYLINDER
      || mType == Type.HIGHRES_TOP_CYLINDER) ? (MeshUtil.isBoundaryVtx (vtx))
         : (mType == Type.PILLOW) ? true
            : (mType == Type.BUD) ? (pnt.z > 0.45) : false;
   }
