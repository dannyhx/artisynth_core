...
public class _Exp_Toolbox {
   public static void trimStrip (
      PolygonalMesh mesh, Point2d stripStart2d, Point2d stripEnd2d,
      double stripRad, double minZ) {
      // Remove faces
      ...
      for (Face face : mesh.getFaces ()) {
         for (Vertex3d fVtx : face.getVertices ()) {
            ...
            Point2d closetStripPt =
               MathUtil
                  .getClosestPtToLineSegment2d (
   ...
   public static void trimVertically (
      PolygonalMesh mesh, double minNodeZ, double maxNodeZ) {
      // Remove faces
      ...
      for (Face face : mesh.getFaces ()) {
         ...
         for (Vertex3d fVtx : face.getVertices ()) {
            if (faceBotVtxPos == null
            || fVtx.getPosition ().z < faceBotVtxPos.z) {
   ...
   public static void drillDown (
      PolygonalMesh mesh, double drillRadius, double drillBotZ,
      double drillTopZ) {
      ArrayList<Face> facesToRemove = new ArrayList<Face> ();
      for (Face face : mesh.getFaces ()) {
         for (Vertex3d fVtx : face.getVertices ()) {
   ...
   public static void trimRadius (
      PolygonalMesh mesh, double minRadius, double maxRadius) {
      ...
      for (Face face : mesh.getFaces ()) {
         ...
         Point2d faceBotVtxPos2d =
            new Point2d (faceBotVtxPos.x, faceBotVtxPos.y);

         if (faceBotVtxPos2d.norm () < minRadius
         || faceBotVtxPos2d.norm () > maxRadius)
            facesToRemove.add (face);
      }
   ...
   public static void trimRangeY (
      PolygonalMesh mesh, double minNodeY, double maxNodeY) {
      ArrayList<Face> facesToRemove = new ArrayList<Face> ();
      for (Face face : mesh.getFaces ()) {
   ...
   public static void trimHorizontallyX (
      PolygonalMesh mesh, double minNodeX, double maxNodeX) {
      ArrayList<Face> facesToRemove = new ArrayList<Face> ();
      for (Face face : mesh.getFaces ()) {
