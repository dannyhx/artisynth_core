...
public class RemeshOps {
    ...
    * 
    * @return Information regarding the bisection (e.g. new faces that are
    * created).
    */
   public OpRv bisectEdge (HalfEdge he) {
      ...
      rv.mAddedFaces[0] =
         MeshUtil.createFace (locEleOppVtx, rv.mNewVertex, vtxH, locFace);
      rv.mAddedFaces[1] =
         MeshUtil.createFace (locEleOppVtx, vtxT, rv.mNewVertex, locFace);
      ...
      if (oppFace != null) {
         ...
         rv.mAddedFaces[2] =
            MeshUtil.createFace (oppEleOppVtx, vtxH, rv.mNewVertex, oppFace);
         rv.mAddedFaces[3] =
            MeshUtil.createFace (oppEleOppVtx, rv.mNewVertex, vtxT, oppFace);
    ...
    * 
    * @return Information regarding the flip (e.g. new faces that are created).
    */
   public OpRv flip (HalfEdge edge) {
      ...
      rv.mRemovedEdge =
         MeshUtil.getCommonEdge (rv.mRemovedFaces[0], rv.mRemovedFaces[1]);
      ...
      rv.mAddedFaces[0] =
         MeshUtil
            .createFace (
               quadVtxs[0], quadVtxs[1], quadVtxs[3], rv.mRemovedFaces[0]);
      rv.mAddedFaces[1] =
         MeshUtil
            .createFace (
               quadVtxs[3], quadVtxs[1], quadVtxs[2], rv.mRemovedFaces[0]);

      addFace (rv.mAddedFaces[0], rv);
      addFace (rv.mAddedFaces[1], rv);

      rv.mAddedEdge =
         MeshUtil.getCommonEdge (rv.mAddedFaces[0], rv.mAddedFaces[1]);
      ...
      for (Face faceB : faceRingB) {
         ...
         for (int i = 0; i < 3; i++) {
            if (triVtxs[i] == b) {
               ...
               Face newFace =
                  MeshUtil
                     .createFace (triVtxs[0], triVtxs[1], triVtxs[2], refNrm);
