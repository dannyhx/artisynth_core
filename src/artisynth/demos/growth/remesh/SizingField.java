 ...
 * 
 * - New face rest area < Old face rest area AND new face rest area <
 * 0.1*mSizeMin^2
 * 
 * - New face rest aspect ratio < mAspectMin
 * 
 * sizeMin :: maximum s sizeMax :: minimum s
 */
public class SizingField {
   ...
   /**
    * Targeted ratio between the minimum and maximum edge length of a triangle.
    * 0.5 is recommended to preserve symmetry.
    */
   ...
   /**
    * Nodes flagged with this will have their elements' sizing field be locked
    * to mSizeUniform.
    */
   ...
   public void computeVertexSizingFields () {
      ...
      HashMap<ShellElement3d,Integer> e2iMap =
         new HashMap<ShellElement3d,Integer> ();
      ...
      for (int n = 0; n < mFemModel.numNodes (); n++) {
         ...
         for (ShellElement3d adjEle : node.getAdjacentShellElements ()) {
            double eleArea =
               ShellUtil.area (adjEle.getNodes (), true /* isRest */); // <MS>
    ...
    * 
    * Amount of refinement needed is increased by: - 1st curvature metric ::
    * depends on dihedral angles. - 2nd curvature metric :: depends on vertex
    * normals. - Velocity metric. - Buckling tendency metric :: Relies of Fe*Fp
    * in compression metric Essentially a modified version of the buckling
    * suppression metric that promotes buckling/refinement instead. - SKIP:
    * Obstruction - SKIP: Fracture
    * 
    * Follows dynamicremesh.cpp :: compute_face_sizing(). Element's rest
    * position == <PS> Element's world position == <WS>
    * 
    * @param restFace
    * Face to be measured, and having a corresponding shell element.
    * 
    * @return 3x3 sizing field matrix of the face.
    */
   protected Matrix3d createFaceSizingField (Face restFace) {
      ShellTriElement ele =
         (ShellTriElement)mFemModel.getShellElement (restFace.idx);
      ...
      Matrix3x2 UV = new Matrix3x2 (); // Basis matrix with 3rd column
                                       // discarded.
      UV
         .set (
            new double[] { base.m00, base.m01, base.m10, base.m11, base.m20,
                           base.m21 });
      ...
      Matrix3d _nrm_J =
         ShellUtil
            .createCustomJacobian (
               ipnt, nodes,
               new Vector3d[] { ShellUtil.getNormal (nodes[0], false),
      ...
      Matrix3d _vel_J =
         ShellUtil
            .createCustomJacobian (
               ipnt, nodes,
               new Vector3d[] { nodes[0].getVelocity (),
                                nodes[1].getVelocity (),
                                nodes[2].getVelocity () },
               null);
      V.mul (_vel_J, invJ0);
      ...
      Matrix2d Mcomp =
         compressionMetric (_F, _Sw2t_Sw2, UV_t, UV, mRefineCompression);
      ...
      for (int i = 0; i < 2; i++) {
         eigValsBuf[i] =
            MathUtil
               .clamp (
                  eigValsBuf[i], 1.0 / pow (mSizeMax, 2),
                  1.0 / pow (mSizeMin, 2));
      }
      ...
      s.set (eigVecs);
      s.mul (new Matrix2d (eigValsBuf[0], 0, 0, eigValsBuf[1]));
   ...
   /**
    * Compute a 3x3 basis matrix that can project a vector or matrix into the
   ...
   protected static Matrix3d localBase (Vector3d normal) {
      ...
      Vector3d u =
         (normal.dot (Vector3d.X_UNIT) > normal.dot (Vector3d.Z_UNIT))
            ? new Vector3d (Vector3d.Z_UNIT) : new Vector3d (Vector3d.X_UNIT);
    ...
    * face.
    */
   protected Matrix2d projectedCurvature (
      Face face, double msArea, boolean isPlasticSpace, Matrix2x3 base) {
      if (DEBUG && msArea <= 0) {
    ...
    *
    * Vector along the intersection is specified by p0 and p1. Plane normals are
    * n0 and n1.
    */
   public double dihedralAngle (
      Vector3d p0, Vector3d p1, Vector3d n0, Vector3d n1) {
   ...
   public double dihedralAngle (HalfEdge edge, boolean isPlasticSpace) {
      ...
      ShellTriElement ele =
         (ShellTriElement)mFemModel.getShellElement (edge.getFace ().idx);
      ShellTriElement oppEle =
         (ShellTriElement)mFemModel
            .getShellElement (edge.getOppositeFace ().idx);
    ...
    * 
    * @param S2
    * 2nd curvature metric, in symmetric form (i.e. S' S). Within 3D space (i.e.
    * 3x3 matrix).
    * 
    * @param UV_t
    * Transpose of UV.
    ...
    */
   protected Matrix2d compressionMetric (
      Matrix3d F, Matrix3d S2, Matrix2x3 UV_t, Matrix3x2 UV, double c) {
      // 3x3 = F' F
