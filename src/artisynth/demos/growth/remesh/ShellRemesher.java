...
public class ShellRemesher extends ShellRemeshOps {
   ...
   /**
    * Whenever an edge is collapsed, a new node should be created that is the
    * average of the edge's parent nodes.
    */
    ...
    * 
    * @param mesh
    * Represents the front surface of the FEM-Shell model. Required to traverse
    * through the topology of the model.
    * 
    * @param femModel
   ...
   double refineComp, double refineVelocity) {
      ...
      mSizingField =
         new SizingField (
            mesh, femModel, sizeMin, sizeMax, aspectMin, refineAngle,
            refineComp, refineVelocity);
   }
   ...
   public void remesh () {
      ...
      while (SF_splitWorstEdge (
         MeshUtil.getAllEdgesViaCopy (mMesh.getFaces ())))
         ;
    ...
    */
   protected void SF_flipEdges (
      ArrayList<Face> activeFaces, LinkedList<HalfEdge> edgesToUpdate) {
      int N = 3 * activeFaces.size ();
      for (int i = 0; i < N; i++) {
         if (isDebug)
            System.out
               .printf ("SF_flipEdges() :: " + "Iteration: %d/%d\n", i, N);
    ...
    * 
    * @param activeFaces
    * Faces that can be modified. Used to prevent duplicate flips. This is will
    * be incremented and decrement as faces are added and removed.
    * 
    * @param edgesToUpdate
    ...
    * 
    * @return True if at least one edge was flipped.
    */
   protected boolean SF_flipSomeEdges (
      ArrayList<Face> activeFaces, LinkedList<HalfEdge> edgesToUpdate,
      int iter) {
      LinkedList<HalfEdge> edges =
         SF_independentEdges (SF_findEdgesToFlip (activeFaces));
    ...
    * 
    * @return Flippable edges.
    */
   protected LinkedList<HalfEdge> SF_findEdgesToFlip (
      ArrayList<Face> activeFaces) {
   ...
   protected boolean SF_shouldFlip (HalfEdge edge) {
      ...
      // do not perform.
      // if ( MathUtil.compare (MathUtil.area (w, y, x), 0,
      // mSF_magicMinFlipArea) == 0 ||
      // MathUtil.compare (MathUtil.area (w, y, z), 0, mSF_magicMinFlipArea) ==
      // 0 ) {
      // return false;
      // }
      ...
      if (abs (
         mSizingField
            .dihedralAngle (edge, false /* isPlasticSpace */)) >= maxAngle) {
         f1++;
         return false;
      }
      ...
      if (abs (
         mSizingField
            .dihedralAngle (w, y, flipped_n0, flipped_n1)) >= maxAngle) {
         f2++;
         return false;
      }
      ...
      Vector2d At_x = new Vector2d ();
      MathUtil.mul (At, x, At_x);
      Vector2d At_y = new Vector2d ();
      MathUtil.mul (At, y, At_y);
      Vector2d At_z = new Vector2d ();
      MathUtil.mul (At, z, At_z);
      Vector2d At_w = new Vector2d ();
      MathUtil.mul (At, w, At_w);
   ...
   /**
    * Given the 4 sides of 2 adjacent faces (projected to 2D plane), should edge
    * be flipped?
    */
   protected boolean SF_shouldFlip2 (
      Vector2d x, Vector2d y, Vector2d z, Vector2d w, Matrix2d M) {
      ...
      return area0 * x_w.dot (M_z_w)
      + area1 * z_y.dot (M_x_y) < -mSF_magicFlipThreshold * (area0 + area1);
   }
    ...
    * 
    * @return true if some "bad" edges remaining.
    */
   protected boolean SF_splitWorstEdge (LinkedList<HalfEdge> edges) {
      ...
      for (int e = 0; e < badEdges.size (); e++) {
         ...
         if (isDebug)
            System.out
               .printf (
                  "BisectEdge: %d-%d\n", badEdge.head.getIndex (),
                  badEdge.tail.getIndex ());
         OpRv rv = bisectEdge (badEdge);

         if (isDebug)
            System.out
               .println ("New bisect node: " + rv.mNewVertex.getIndex ());
    ...
    * 
    * @return Bad edges, sorted from largest edge "size".
    */
   protected LinkedList<HalfEdge> SF_findBadEdges (LinkedList<HalfEdge> edges) {
      ...
      Collections
         .sort (oversizedEdges, new Comparator<Pair<Double,HalfEdge>> () {
            @Override
            public int compare (
    ...
    * 
    * @return true if an edge was collapsed.
    */
    ...
    * 
    * @return Details regarding the collapse.
    */
    ...
    * occur. This is overridden when `isCollapseEdge_viaAvg` is true; both
    * vertices of the edges are replaced with an average vertex.
    */
         ...
         for (Face face : MeshUtil.getOneRingFaces (curEdgeVtx)) {
            ...
            for (int v = 0; v < 3; v++) {
               ...
               triVtxsRest[v]
                  .setPosition (
                     mFemModel
                        .getNode (triVtxs[v].getIndex ()).getRestPosition ());
               triVtxsRest[v].setIndex (triVtxs[v].getIndex ());
            }

            // Compute the rest area of face
            Point3d[] triRPos =
               new Point3d[] { triVtxsRest[0].pnt, triVtxsRest[1].pnt,
                               triVtxsRest[2].pnt };
            ...
            if (isCollapseEdge_viaAvg) {
               curVtxReplacee.pnt =
                  MathUtil.avg (curEdgeVtxRest.pnt, otherEdgeVtxRest.pnt);
            }
            else {
            ...
            || (asp < mSizingField.mAspectMin)) {
               ...
               if (allowCollapseForAlreadyPoorAspEdges
               && asp0 < mSizingField.mAspectMin && asp > asp0) {
    ...
    */
   protected LinkedList<HalfEdge> SF_independentEdges (
      LinkedList<HalfEdge> edges) {
    ...
    * sizing fields.
    * 
    * @return If above 1, the edge should be remeshed.
    */
   ...
   protected double edgeMetric (int n0, int n1) {
      ...
      Vector3d du =
         new Vector3d ()
            .sub (node0.getRestPosition (), node1.getRestPosition ());
   ...
   protected double edgeMetric_usingAvgOf2nd (int n0, int n1_x, int n1_y) {
      ...
      Point3d node1Rest =
         MathUtil.avg (node1_x.getRestPosition (), node1_y.getRestPosition ());

      Vector3d du = new Vector3d ().sub (node0.getRestPosition (), node1Rest);
      ...
      Matrix3d S1 =
         MathUtil
            .avg (
               mSizingField.getVertexSF (n1_x),
               mSizingField.getVertexSF (n1_y));
      S1.mul (S1_du, du);
   ...
   protected void replace (
      Vertex3d thisVtx, Vertex3d withThis, Vertex3d[] inThisArray) {
   ...
   protected double aspect (Point3d u0, Point3d u1, Point3d u2) {
      ...
      return 12 * sqrt (3) * 0.5
      * new Vector3d ()
         .cross (new Vector3d ().sub (u1, u0), new Vector3d ().sub (u2, u0))
         .norm ()
      / pow (perimeter, 2);
   }
