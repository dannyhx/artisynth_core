...
/**
 * Remeshing operations (e.g. edge bisect, flip, and collapse) that operate on
 * FEM-Shell models.
 * 
...
public class ShellRemeshOps extends RemeshOps {
    ...
    * have to interpolated, and indirect node neighbors don't have to be
    * cleared.
    */
   ...
   public ShellRemeshOps (PolygonalMesh mesh, FemModel3d femModel) {
      ...
      if (mFemModel.numShellElements () > 0) {
         ...
         mHasBackNode =
            femModel
               .getShellElement (0).getElementClass () == ElementClass.SHELL;
      }
   }
   ...
   protected void setVertexViaLinearInterpolation (
      ...
      DataBuffer buf_newNode =
         linearInterpolateDataBuffer (buf_n0, buf_n1, 0.5);
   ...
   /**
    * Given two node, create an average node, and add the average node to the
    * model.
    */
   protected Vertex3d addVertexViaLinearInterpolation (
   ...
   protected Object addFace (Face face, OpRv opRv) {
      ...
      if (mFemModel.myThinShellAux == null) {
         ...
         Pair<Matrix3d,Matrix3d> strainPair =
            sOpRv.getAvgParentPlasticStrains ();
         ...
         Matrix3d plasMembStrain =
            sOpRv.mParentPlasticMembStrains.removeFirst ();
         Matrix3d plasBendStrain =
            sOpRv.mParentPlasticBendStrains.removeFirst ();
         newEle.getPlasticDeformation ().set (plasMembStrain);
         newEle.getPlasticBendStrain ().set (plasBendStrain);
      }
   ...
   public Object removeFace (Face face, OpRv opRv) {
      ShellTriElement rmEle =
         (ShellTriElement)mFemModel.getShellElement (face.idx);
   ...
   protected ShellTriElement createElement (Face face) {
      ...
      isEleModified = true;
      ShellTriElement newEle =
         createElement (triNodes[0], triNodes[1], triNodes[2], mShellThickness);
   ...
   protected ShellTriElement createElement (
    ...
    */
   protected DataBuffer linearInterpolateDataBuffer (
      DataBuffer A, DataBuffer B, double s) {
    ...
    * New Node
    */
   protected void createInterpolatedConstraint (
      Vertex3d vtxA, Vertex3d vtxB, Vertex3d vtxC) {
      ...
      for (CollisionHandler colHdlr : mCCAgg.myColHdlrs) {
         for (boolean isBackNode : new boolean[] { false, true }) {
            ...
            // Constraint specific
            // Vector3d nrm_avg = new Vector3d();// Nrm calc as if for single
            // mast.

            // double dist_avg = 0; // Distance calculated as if for single
            // master.
            ...
            cc_avg.myMasters0
               .add (createSingleNodeVertexContactMaster (nodeC_fb, 1));
            cc_avg.myMasters1
               .add (createSingleNodeVertexContactMaster (nodeC_fb, 1));
            cc_avg.m =
               (n0_ccs.size () > 0) ? n0_ccs.get (0).m : n1_ccs.get (0).m;
   ...
   /**
    * Remove the constraint masters that are associated with the vertex. The
    * vertex is typically the vertex that's to be removed from the mesh.
    */
   ...
   protected LinkedList<ContactConstraint> getFilteredContactConstraints (
      ...
      LinkedList<ContactConstraint> n_ccs =
         new LinkedList<ContactConstraint> ();

      // CSNCMT
      for (ContactConstraint cc : colHdlr.myUnilaterals) {
         ContactConstraint filtered_cc =
            getFilteredContactMasters (node, cc, isBackNode);
         if (filtered_cc.myMasters0.size () > 0
         || filtered_cc.myMasters1.size () > 0) {
            n_ccs.add (cc);
         }
      }
   ...
   /**
    * Get contact masters (if any) within the given contact constraint cc that
    * are associated with the given node.
    * 
    * Recall that contact masters are usually instances of VertexContactMaster,
    * which either involves one node or 3 nodes.
    * 
    * @return ContactConstraint with myMasters0 and myMasters1 populated. Each
    * myMasters list will contain 0 or 1 VertexContactMaster. In turn, every
    * VertexContactMaster will have a PointParticleAttachment (filtered from the
    * 1 or 3 nodes), which identifies the interested node, and associated
    * weight.
    */
   protected ContactConstraint getFilteredContactMasters (
      FemNode3d node, ContactConstraint cc, boolean isBackNode) {
      ...
      for (int m = 0; m < 2; m++) {
         ...
         ArrayList<ContactMaster> n_masters =
            (m == 0) ? n_cc.myMasters0 : n_cc.myMasters1;
               ...
               for (ArrayList<ContactMaster> vcm_cms : vcm.myMasterLists) {
                  ...
                  for (ContactMaster vcm_cm : vcm_cms) {
                     if (vcm_cm instanceof PointParticleAttachment) {
                        ...
                        if (!isBackNode && particle == node
                        || isBackNode && particle == backNode) {
                           VertexContactMaster n_vcm =
                              new VertexContactMaster ();
                           n_vcm.myMasterLists =
                              new ArrayList[] { new ArrayList<ContactMaster> () };
   ...
   protected VertexContactMaster createSingleNodeVertexContactMaster (
      Particle node, double wt) {
