      ...
      if (myDirectorRenderLen > 0) {
         ...
         for (FemNode3d n : myNodes) {
            if (n.hasDirector ()) {
               ...
               renderer.addVertex (x[0], x[1], x[2]);
               renderer.addVertex (y[0], y[1], y[2]);
            }
         }
         renderer.endDraw ();
      }
         ...
         for (int k = 0; k < ipnts.length; k++) {
            ...
            for (int en = 0; en < gEle.numNodes (); en++) {
               ...
               if (a != 0) {
                  ...
                  nodalFgs[nodeGlobalIdx]
                     .scaledAdd (
                        a / node.numAdjacentElements (),
                        idata[k].getFp () != null ? idata[k].getFp ()
                           : Matrix3d.IDENTITY);
               }
            }
         }
   ...
   public double[] getNodalResidualPlasticBendingStrain () {
      ...
      for (FemElement3dBase ele : this.getAllElements ()) {
         ...
         for (int k = 0; k < ipnts.length; k++) {
            ...
            // Calculate F
            double detInvJ0 =
               ipnts[k].computeInverseRestJacobian (invJ0, gEle.getNodes ());
            if (detInvJ0 <= 0) {
   ...
   public void addSolveBlocks (SparseNumberedBlockMatrix S) {
      ...
      for (int i = 0; i < myNodes.size (); i++) {
         ...
         for (FemNodeNeighbor nbr : getIndirectNeighbors (node)) {
            if (nbr
               .getNode ().checkFlag (StiffnessMatrixUtil.EDGE_DELEGATE_FLAG)) {
               continue;
            }
            nbr.addSolveBlocks (S, node);
         }
      ...
      for (int i = 0; i < myNodes.size (); i++) {
         ...
         if (node.getLocalSolveIndex () != -1) {
            ...
            for (FemNodeNeighbor nbr : getIndirectNeighbors (node)) {
               ...
               if (nbr
                  .getNode ()
                  .checkFlag (StiffnessMatrixUtil.EDGE_DELEGATE_FLAG)) {
                  continue;
               }
