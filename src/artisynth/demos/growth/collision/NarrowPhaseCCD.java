...
public class NarrowPhaseCCD {
   ...
   /**
    * Measure to see if a traversing vertex and traversing triangle will
    * collide.
    * 
    * @param p0
    ...
    * 
    * @return Details regarding the collision.
    */
   public CCRV collideVertexTrianglePnts (
      Vector3d p0, Vector3d p1, Vector3d a0, Vector3d a1, Vector3d b0,
      Vector3d b1, Vector3d c0, Vector3d c1, double epsilon) {
   ...
   public CCRV collideVertexTriangleDiff (
      Vector3d p0, Vector3d pdiff, Vector3d a0, Vector3d adiff, Vector3d b0,
      Vector3d bdiff, Vector3d c0, Vector3d cdiff, double epsilon) {
      ...
      /*
       * Quadratic Equation Qt^2 + Rt + S = AB(t)^AC(t) which calculates vector
       * orthogonal to vectors AB and AC at time t.
       */
       ...
       * AB and AC correspond to moments of coplanarity of vertices p, a, b, c.
       * We take the dot-product of the vector quadratic equation above with the
       * vector AP(t), producing a scalar cubic equation, with roots
       * corresponding to moments of coplanarity
       *
       * Cubic Equation At^3 + Bt^2 + Ct + D = 0 for t values that make all
       * points coplanar
       */
   ...
   public CCRV collideEdgeEdgePnts (
      Vector3d a0, Vector3d a1, Vector3d b0, Vector3d b1, Vector3d c0,
      Vector3d c1, Vector3d d0, Vector3d d1, double epsilon) {
   ...
   public CCRV collideEdgeEdgeDiff (
      Vector3d a0, Vector3d adiff, Vector3d b0, Vector3d bdiff, Vector3d c0,
      Vector3d cdiff, Vector3d d0, Vector3d ddiff, double epsilon) {
      ...
      /*
       * Quadratic Equation Qt^2 + Rt + S = AB(t)^CD(t) which calculates vector
       * orthogonal to vectors AB and CD at time t.
       */
      makeCrossProductQuadraticFromRays (
       ...
       * AB and CD correspond to moments of coplanarity of vertices a, b, c, d.
       * We take the dot-product of the vector quadratic equation above with the
       * vector AC(t), producing a scalar cubic equation, with roots
       * corresponding to moments of coplanarity
       *
       * Cubic Equation At^3 + Bt^2 + Ct + D = 0 for t values that make all
       * points coplanar
   ...
   void makeCrossProductQuadraticFromRays (
      Vector3d c0, Vector3d c1, Vector3d c2, Vector3d oa, Vector3d da,
      Vector3d ob, Vector3d db) {

      c0
         .set (
            oa.y * ob.z - oa.z * ob.y, oa.z * ob.x - oa.x * ob.z,
            oa.x * ob.y - oa.y * ob.x);

      c1
         .set (
            oa.y * db.z + da.y * ob.z - oa.z * db.y - da.z * ob.y,
            oa.z * db.x + da.z * ob.x - oa.x * db.z - da.x * ob.z,
            oa.x * db.y + da.x * ob.y - oa.y * db.x - da.y * ob.x);

      c2
         .set (
            da.y * db.z - da.z * db.y, da.z * db.x - da.x * db.z,
            da.x * db.y - da.y * db.x);

   }

   boolean computeBarycentricCoords (
      Vector2d bary, Vector3d p, Vector3d a, Vector3d b, Vector3d c,
      double epsilon) {
    ...
    * 
    * @param t
    * Time within the traversal of both edges that the closest points should be
    * determined. [0,1].
    * 
    * @param out_e0
   ...
   boolean cubicRootsMayBeInZeroToOne (double a, double b, double c, double d) {
      ...
      if (sgnA == Math.signum (b) && sgnA == Math.signum (c)
      && sgnA == Math.signum (d)) {
         return false;
      }
      // d,c > 0 and d + c > |a| + |b|
