...
public class SweptMeshInfo {
    ...
    * 
    * This allows BVTree intersection methods to rely on a single matrix (i.e.
    * provided by myMesh.getMeshToWorld()) only, instead of needing to refactor
    * the BVTree to account for 2 different transformation matrices to obtain
    * either the previous and current world-space vertex positions.
    * 
    * The comments in {@link #updatePrevPositionsX0()} provides more details
    * regarding this property.
    */
   public Point3d[] prevPositionsX0;

   /**
    * Matrix that can transform {@link #previousPositions} from local-space to
    * the previous world-space. In other words, this is the "to world-space"
    * transformation matrix that was used in the previous time step.
    */
   ...
   public void build (PolygonalMesh mesh) {
      ...
      for (int i = 0; i < numv; i++) {
         sweptVertices[i] =
            new SweptVertex (mesh.getVertex (i), prevPositionsX0);
      }
    ...
    * positions in world-space when the current "to world-space" transformation
    * matrix is applied.
    */
