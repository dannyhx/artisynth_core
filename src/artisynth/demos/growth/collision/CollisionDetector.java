 ...
 * 
 * DEV NOTES: Artisynth AABB tree runs faster if bvhWorld set to Identity, which
 * is ok for FEM models. - Currently using intersectTreeWorldIdentity()
 */
public class CollisionDetector {
   ...
   public static double myClothThickness = 1e-2; // Should correspond to
                                                 // penetrationTol
   ...
   /**
    * Detect nearby (NEARBY) or colliding (ACTUAL) features? (ACTUAL_ZONED) is
    * the same as (ACTUAL) except impact zones are used.
    */
   public static Stage mStage = Stage.ACTUAL;

   public enum Stage {
      NEARBY, ACTUAL, ACTUAL_ZONED
   };
   ...
   /**
    * Max number of iterations to perform continuous detection and resolution in
    * order to eliminate secondary collisions.
    */
   ...
   /**
    * Maintains a reference to the unilaterals that were used to undo the
    * penetration points. These unilaterals are adjusted during remeshing before
    * they're applied to the subsequent constraint backward Euler solve.
    */
    ...
    * 
    * @return Info regarding each detected collision.
    */
   ...
   protected boolean isBodyDynamic (CollidableBody body) {
      if (body instanceof RigidBody) {
         ...
         FemModel3d femModel =
            (FemModel3d)femMeshComp.getParent ().getParent ();
    ...
    * Is mesh1 dynamic?
    * 
    * @return Info regarding any collisions detected.
    */
   protected ContactInfo getContacts (
      SweptMeshInfo smi0, SweptMeshInfo smi1, boolean isDynamic0,
      boolean isDynamic1) {
      ...
      ArrayList<PenetratingPoint> pentPts1 =
         (smi0 != smi1) ? findPenetratingPoints (smi1, smi0)
    ...
    * hitTime. - Vertex-Tri collision where a connected edge of the vertex has
    * an earlier hitTime.
    */
   protected void removeRedundantCollisions (ContactInfo cinfo) {
      ...
      cinfo
         .getEdgeEdgeContacts ().removeIf (
            eeCt -> isExistEarlierVtxTriContact (
      ...
      for (int m = 0; m < 2; m++) {
         final int fm = m;
         cinfo
            .getPenetratingPoints (fm).removeIf (
               pentPt -> isExistEarlierEdgeEdgeContact (
                  fm, cinfo.getEdgeEdgeContacts (), pentPt.vertex,
                  pentPt.hitTime));
      }
      ...
      cinfo
         .getEdgeEdgeContacts ().removeIf (
            eeCt -> isExistingInverseEdgeEdgeContact (
               eeCt, cinfo.getEdgeEdgeContacts ())
    ...
    * Space-time path of mesh1.
    * 
    * @return Detected vertex-triangle collisions.
    */
   protected ArrayList<PenetratingPoint> findPenetratingPoints (
      ...
      smi0.myVertexTree
         .intersectTreeWorldIdentity (
            ixtVertexNodes, ixtFaceNodes, smi1.myTriangleTree);
      ...
      for (int i = 0; i < ixtVertexNodes.size (); i++) {
         SweptVertex sv = (SweptVertex)ixtVertexNodes.get (i).getElements ()[0];
         SweptTriangle st =
            (SweptTriangle)ixtFaceNodes.get (i).getElements ()[0];
         ...
         // Case 1: This is the vertex's first collision
            ...
            Pair<Boundable,CCRV> cloesetCsn =
               vtx2triCsns.getFirstCollision (sv);
    ...
    * Space-time path of mesh1.
    * 
    * @return Detected edge-edge collisions.
    */
   protected ArrayList<EdgeEdgeContact> findEdgeEdgeContacts (
      ...
      smi0.myEdgeTree
         .intersectTreeWorldIdentity (
            ixtEdgeNodes0, ixtEdgeNodes1, smi1.myEdgeTree);
      ...
      Boundable2BoundableCollisions e2eCsns =
         new Boundable2BoundableCollisions ();
    ...
    * 
    * @return Details regarding the collision, if any.
    */
   ...
   /**
    * Refine a colliding vertex-triangle info into Artisynth's PenetratingPoint
    * class in order to hook up with its LCP solver.
    */
   protected PenetratingPoint createPenetratingPoint (
      ...
      Point3d fPt_prv =
         MathUtil.projectPoint2plane (v_prv, fPts[0], fPts[1], fPts[2]);

      fPts = st.computeInstanteousTriangle (1);
      Point3d fPt_cur =
         MathUtil.projectPoint2plane (v_cur, fPts[0], fPts[1], fPts[2]);
      ...
      if (myDebug) {
         System.out
            .println (
               "Penetrating Point: " + " V: " + sv.myVertex.getIndex () + " T: "
               + st.myFace.vertexStr () + " FaceNrm: "
               + st.myFace.getWorldNormal ().toString ("%.2f") + " Normal: "
               + pentPt.normal.toString ("%.2f") + " Dist: "
               + String.format ("%.4f", pentPt.distance) + " HitTime: "
               + String.format ("%.4f", pentPt.hitTime) + " \n  " + " f2v_prv: "
               + f2v_prv.toString ("%.8f") + " f2v_cur: "
               + f2v_cur.toString ("%.8f") + " nrm_dot_f2v_prv: "
               + String.format ("%.8f", nrm_dot_f2v_prv) + " nrm_dot_f2v_cur: "
               + String.format ("%.8f", nrm_dot_f2v_cur) + " \n ");
      }
   ...
   /**
    * Refine a colliding edge-edge info into Artisynth's EdgeEdgeContact class
    * in order to hook up with its LCP solver.
    */
   protected EdgeEdgeContact createEdgeEdgeContact (
      ...
      se0
         .computeClosestEdgePairPoints (
            se1, 0, e0_pt_prv, e1_pt_prv, null, mySpaceElipson);

      Point3d e0_pt_cur = new Point3d ();
      Point3d e1_pt_cur = new Point3d ();
      se0
         .computeClosestEdgePairPoints (
            se1, 1, e0_pt_cur, e1_pt_cur, null, mySpaceElipson);
      ...
      eeCt.e0Pnts_justBefore_hitTime =
         se0.computeInstanteousEdgePoints (hitTime);
      eeCt.e1Pnts_justBefore_hitTime =
         se1.computeInstanteousEdgePoints (hitTime);
      ...
      if (myDebug)
         System.out
            .println (
               "Edge-Edge Contact: " + " E0: " + eeCt.edge0.vertexStr ()
               + " E1: " + eeCt.edge1.vertexStr () + " e10_pt_prv: "
               + e10_pt_prv.toString ("%.4f") + " dot: " + cross_dot_e10_prv
               + " e10_pt_cur: " + e10_pt_cur.toString ("%.4f") + " dot: "
               + cross_dot_e10_cur + " e01crossbp: "
               + e01crossbp.toString ("%.4f") + " e01cross: "
               + e01cross.toString ("%.4f") + " disp: " + eeCt.displacement
               + "\n");
   ...
   protected void printCollisions (ContactInfo cinfo) {
      for (EdgeEdgeContact eeCt : cinfo.getEdgeEdgeContacts ())
         System.out
            .printf (
               "  Edge-Edge Collision: %s-%s. Disp: %.4f, "
               + "HitTime: %.4f, Normal: %s\n", eeCt.edge0.vertexStr (),
               eeCt.edge1.vertexStr (), eeCt.displacement, eeCt.hitTime,
               eeCt.point1ToPoint0Normal.toString ("%.2f"));
            ...
            String faceVtxStr =
               (fmc1 != null) ? String
                  .format (
                     "[%d-%d-%d]",
                     fmc1
                        .getNodeForVertex (pentPt.face.getVertex (0))
                        .getNumber (),
                     fmc1
                        .getNodeForVertex (pentPt.face.getVertex (1))
                        .getNumber (),
                     fmc1
                        .getNodeForVertex (pentPt.face.getVertex (2))
                        .getNumber ())
                  : pentPt.face.vertexStr ();

            System.out
               .printf (
                  "  Vtx-Tri Collision: [ %d ]-%s. Disp: %.4f, "
                  + "HitTime: %.4f, Nrm: %s\n", v, faceVtxStr, pentPt.distance,
                  pentPt.hitTime, pentPt.normal.toString ("%.2f"));
         }
   }

   /**
    * Given a colliding edge, does one of its vertex have an earlier collision
    * time?
    * 
    * @param pentPts
   ...
   protected boolean isExistEarlierVtxTriContact (
      ...
      for (PenetratingPoint pentPt : pentPts) {
         boolean isEdgeVtxConn =
            MeshUtil.isEdgeAndVtxConnected (edge, pentPt.vertex);
         ...
         if (isEdgeVtxConn && vtxTri_hitTime < edge_hitTime - myTimeElipson) {
            if (myDebug)
               System.out
                  .printf (
                     "Skipping edge [%d-%d] b/c pentPt [%d] has "
                     + "earlier hitTime.\n", edge.head.getIndex (),
   ...
   /**
    * Given a colliding vertex, does one of its connected edges have an earlier
    * collision time?
    * 
    * @param m
    * Mesh0 or Mesh1?
    ...
    */
   protected boolean isExistEarlierEdgeEdgeContact (
      int m, ArrayList<EdgeEdgeContact> eeCts, Vertex3d pentVtx,
      double pentVtx_hitTime) {
      for (EdgeEdgeContact eeCt : eeCts) {
         boolean isEdgeVtxConn =
            (m == 0) ? MeshUtil.isEdgeAndVtxConnected (eeCt.edge0, pentVtx)
         ...
         && edgeEdge_hitTime < pentVtx_hitTime - myTimeElipson) {
            if (myDebug)
               System.out
                  .printf (
                     "Skipping pentPt [%d] b/c edge-edge "
                     + "[%d-%d][%d-%d] has earlier hitTime.\n",
                     pentVtx.getIndex (), eeCt.edge0.head.getIndex (),
                     eeCt.edge0.tail.getIndex (), eeCt.edge1.head.getIndex (),
                     eeCt.edge1.tail.getIndex ());
   ...
   /**
    * For each vertex or edge, keep an organized list of its collisions (e.g.
    * vertex-triangle or edge-edge).
    * 
    * Used for filtering out redundant collisions.
    */
   public class Boundable2BoundableCollisions {
      public LinkedHashMap<Boundable,ArrayList<Pair<Boundable,CCRV>>> myBnd2BndCsns;
   ...
   protected CCRV isCollisionImminent (SweptVertex sv, SweptTriangle st) {
      ...
      Point3d closestTriPnt =
         MathUtil
            .projectPoint2plane (
               svPntsW[sv.V0], stPntsW[st.A0], stPntsW[st.B0], stPntsW[st.C0]);
      ...
      myContCol
         .computeBarycentricCoords (
            rv.bary, closestTriPnt, stPntsW[st.A0], stPntsW[st.B0],
            stPntsW[st.C0], mySpaceElipson);
   ...
   /**
    * Refine a nearby vertex-triangle info into Artisynth's PenetratingPoint
    * class in order to hook up with its LCP solver.
    */
   protected PenetratingPoint createImminentPenetratingPoint (
      ...
      // double overlapImpulse = myTimestep * mySpringStiffness *
      // ccrv.thicknessPenetration;
      //
      // // Limit velocity change to 10% of thickness overlap.
      // // Mass will be factored in MLPC step.
      // overlapImpulse = Math.min(
      // overlapImpulse,
      //// myAvgNodeMass * // Node mass will be accounted in the mass block
      // matrix.
      ...
      Point3d closestTriPnt =
         MathUtil
            .linearInterpolation (
               stPntsW[st.A1], stPntsW[st.B1], stPntsW[st.C1], ccrv.bary.x,
               ccrv.bary.y);
   ...
   protected CCRV isCollisionImminent (SweptEdge se0, SweptEdge se1) {
      ...
      if (thicknessPenetration < 0) {
         ...
         // For imminent collisions, the hitTime doesn't actually exist.
         // We'll put the distance between the imminent features here.
         // When redundant collisions are being filtered out, collisions with
         // the
         // earliest hitTimes are prioritized. For imminent collisions, we'll
         // prioritize imminent collisions that are closer to each other.
         rv.hitTime = sizedNrm.norm ();
   ...
   /**
    * Refine a nearby edge-edge info into Artisynth's PenetratingPoint class in
    * order to hook up with its LCP solver.
    */
