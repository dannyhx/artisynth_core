 ...
 * 
 * TODO: Assumes a single morphogen type that diffuses (TYPE-4). The absorbed
 * TYPE-4 morphogen is simply converted into PAR (TYPE-1) and PER (TYPE-2).
 * 
 * TODO: Need to refactor to handle bending-subtype morphogen. Currently,
...
public class Morphogen2GrowthTensor {
   ...
   /**
    * At each node, convert a fraction of the diffusable morphogen (Type-4) into
    * morphogen PAR (Type-1) and PER (Type-2), which are used to generate the
    * growth tensor.
    * 
    * @param fraction
    * [0.00 to 1.00] of diffusable morphogen to convert into its activate form
    * (PAR + PER).
    */
   public void activateFractionOfMorphogen (
      double fraction, boolean isActivatePAR, boolean isActivatePER,
      boolean isActivateNOR) {
      for (FemNode3d node : mFemModel.getNodes ()) {
         ...
         if (isActivatePAR)
            chems[GrowChemical.PAR.mIdx] += xferChem;
         if (isActivatePER)
            chems[GrowChemical.PER.mIdx] += xferChem;
         if (isActivateNOR)
            chems[GrowChemical.NOR.mIdx] += xferChem;
   ...
   /**
    * For each element, compute its growth tensor, which is dependent on the
    * morphogen concentration of its nodes.
    * 
    * Growth tensor: M[i][j] i => PAR, PER, NOR j => node index.
    */
   ...
   /**
    * For each element, interpolate its rotated growth tensor to the each
    * integration point.
    * 
    * This is only required for solid-shells, or more generally, non-reduced
    * elements.
    */
   ...
   /**
    * For each element, inject its integration growth tensor into its plastic
    * strain (or plastic deformation gradient more specifically) attribute.
    */
      ...
      for (FemElement3dBase ele : ShellUtil.getAllElements (mFemModel)) {
         ...
         for (int k = 0; k < idata.length; k++) {
            ...
            if (this.fixedBendingStrain == null) {
               // Before incrementing Fp, take the existing Fp, undo its
               // rotation
